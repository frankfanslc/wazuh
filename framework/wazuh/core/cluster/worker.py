...
class WorkerHandler(client.AbstractClient, c_common.WazuhCommon):
    ...
    async def sync_extra_valid(self, _: Dict):
        ...
        try:
            ...
            logger.debug(f"Starting sending _ files to master.")
            _sync = SyncFiles(cmd=b'syn_e_w_m', logger=logger, worker=self)
            ...
            await _sync.sync(files_to_sync=files_to_sync, files_metadata=files_to_sync)
            after = datetime.utcnow().timestamp()
            logger.debug(f"Finished sending _ files in {(after - before):.3f}s.")
            logger.info(f"Finished in {(after - self.integrity_sync_status['date_start']):.3f}s.")

        # If exception is raised during sync process, notify the master so it removes the file if received.
        except exception.WazuhException as e:
            logger.error(f"Error synchronizing _ files: {e}")
